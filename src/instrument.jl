...
function instrument(filename::String, expr::Expr)
    return instrument(filename, expr, Val(expr.head))
end

function instrument(filename::String, expr::Expr, lnn::LineNumberNode)
    return instrument(filename, expr, Val(expr.head), lnn)
end

function instrument(filename::String, expr::Expr, ::Val{:block})
    ...
    return instrument(filename, expr, Val(:block), original_args[1])
end
...
function instrument(filename::String,
    ...
    for i = 2:num_args
        new_args[2] = instrument(filename, original_args[2], lnn)
    end
    return Expr(:block,
                tracker(filename, lnn),
